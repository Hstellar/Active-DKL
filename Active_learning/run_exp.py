...
class Optimization(nn.Module):
    ...
    def valid_step(self, batch_size, n_features, x_val, y_val):
        ""
        if (self.mode == "ExactGP") or (self.mode == "DeepGP"):
            x_val = x_val.view([batch_size, n_features]).to(self.device)
        ...
        if self.mode == "ExactGP":
            ...
            z_query = self.feature_extractor(x_val)
    ...
    def evaluate_val(self, X_train, val_loader, batch_size, n_features=1):
        self.model.eval()
        ...
        # self.feature_extractor.train()
            ...
            for x_val, y_val in val_loader:
                ...
                # print(x_val.shape)
                z_query = self.valid_step(batch_size, n_features, x_val, y_val)
                z_query = z_query.to(self.device)
                # print(z_query.shape)
                # print(z_query)
                # print(self.model(z_query))
                yhat = self.likelihood(self.model(z_query))
                alpha = 0.8
                U=np.sqrt(yhat.variance.detach().cpu().numpy()*yhat.mean.detach().cpu().numpy()**2)
                sim = pairwise_distances(x_val, X_train).min(1)
                sim_scores = 1 / (1 + sim)
                U_scores = np.tanh(U)
                scores = alpha * sim_scores + (1 - alpha) * U_scores

                q_ind_ind = np.mean(scores)
                # self.idx_train = np.append(self.idx_train, self.idx_test[q_ind_ind])
                queries.append(q_ind_ind)
                # self.idx_test = np.delete(self.idx_test, q_ind_ind)
            target = np.argmax(queries)
            for batch_idx, data in enumerate(val_loader, target):
                query = data 
                break
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            for x_batch, y_batch in train_loader:
                if (self.mode == "ExactGP") or (self.mode == "DeepGP"):
                    ...
                    query = self.evaluate_val(x_batch, val_loader, batch_size=batch_size, n_features=n_features)
                    query[0] = query[0].view([batch_size, -1, n_features]).to(self.device)
                    query[1] = query[1].to(self.device)

                    
                    
                    x_batch = x_batch.view([batch_size, -1, n_features]).to(self.device)
                    x_batch = torch.cat((x_batch, query[0]), dim=0)

                    y_batch = y_batch.to(self.device)
                    y_batch = torch.cat((y_batch, query[1]))
